...
#------------------------------------------------------------------------------
#Para cada variable , grafico para cada mes la mediana de esa variable
...
{
  tbl   <- dataset[ foto_mes<=202105 ,
                    list( "mediana" = round(median( get(campo), na.rm=TRUE)),0) ,
                    foto_mes ]
...
#------------------------------------------------------------------------------
#Para cada variable , grafico para cada mes el sd de esa variable
...
{
  tbl   <- dataset[ foto_mes<=202105 ,
                    list( "sd" = round(sd( get(campo), na.rm=TRUE)/mean( get(campo), na.rm=TRUE),0)) ,
                    foto_mes ]
...
for( campo in  campos_buenos )
{
  ...
  plot(x= 1:nrow(tbl),
       y= tbl$mediana,
       type= "o",
       main= paste0("Medianas NO cero -  ",  campo),
       xlab= "Periodo",
       ylab= "Mediana valores no cero",
       xaxt= "n"
  )
...
#------------------------------------------------------------------------------
#Para cada variable , grafico para cada mes el sd de esa variable cuando la variable es DISTINTA de cero
...
for( campo in  campos_buenos )
{
  ...
  plot(x= 1:nrow(tbl),
       y= tbl$sd,
       type= "o",
       main= paste0("SD NO cero -  ",  campo),
       xlab= "Periodo",
       ylab= "SD valores no cero",
       xaxt= "n"
  )
